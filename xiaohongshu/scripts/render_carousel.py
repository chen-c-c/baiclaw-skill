...
def _extract_content(article: str, title: str, topics: list) -> dict:
    ...
    if not insight:
        insight = sentences[len(sentences) // 2] if sentences else title
...
def build_html(draft: dict, brand: str = "") -> str:
    title = draft.get("titles", [""])[0]
    ...
    steps_html = "\n".join(
        ...
        f'<div class="c5-step-name">{_e(f[:12])}</div></div>'
...
.c1-eyebrow {{ font-size: 11px; color: {CYAN}; letter-spacing: 4px; font-weight: 600; margin-bottom: 32px; }}
.c1-title {{ font-size: 48px; font-weight: 900; line-height: 1.25; letter-spacing: -1px; }}
.c1-title .accent {{ color: {CYAN}; }}
.c1-title .underline {{ border-bottom: 4px solid {CYAN}; padding-bottom: 2px; display: inline-block; }}
.c1-sub {{ font-size: 16px; color: rgba(255,255,255,0.65); line-height: 1.85; margin-top: 28px; }}
.c1-sub strong {{ color: {WHITE}; }}
.c1-tag {{
  position: absolute; bottom: 24px; right: 24px; z-index: 10;
  font-size: 11px; color: {CYAN}; border: 1px solid rgba(22,192,254,0.4);
  padding: 5px 14px; border-radius: 999px; letter-spacing: 2px; font-weight: 600;
}}
.c1 .watermark {{ color: rgba(255,255,255,0.55); }}

/* C2 - Intro */
.c2 {{ background: {LIGHT}; padding: 60px 48px; color: #0A0A0A; }}
.c2-num {{ font-size: 13px; font-weight: 800; color: {BLUE}; letter-spacing: 3px; margin-bottom: 20px; }}
...
.c3-num {{ font-size: 13px; font-weight: 800; color: {BLUE}; letter-spacing: 3px; margin-bottom: 20px; }}
.c3-headline {{ font-size: 32px; font-weight: 800; line-height: 1.4; margin-bottom: 28px; }}
.c3-headline .quote {{ color: {BLUE}; }}
.scene-card {{ background: {WHITE}; border: 1px solid rgba(26,26,26,0.08); border-radius: 4px; padding: 16px 20px; margin-bottom: 12px; }}
.scene-name {{ font-size: 15px; font-weight: 700; color: #0A0A0A; margin-bottom: 2px; }}
.scene-fail {{ font-size: 13px; color: #888; }}
...
.c4-mark {{ font-size: 56px; color: {CYAN}; opacity: 0.7; line-height: 1; margin-bottom: 20px; }}
.c4-text {{ font-size: 36px; font-weight: 800; line-height: 1.4; letter-spacing: 0.5px; }}
.c4-text .accent {{ color: {CYAN}; }}
.c4-aside {{ font-size: 14px; color: rgba(255,255,255,0.55); margin-top: 24px; }}
.c4 .watermark {{ color: rgba(255,255,255,0.55); }}

/* C5 - Solution */
.c5 {{ background: {LIGHT}; padding: 60px 48px; color: #0A0A0A; }}
.c5-num {{ font-size: 13px; font-weight: 800; color: {BLUE}; letter-spacing: 3px; margin-bottom: 20px; }}
...
.c5-step {{ flex: 1; padding: 14px 8px; background: {WHITE}; border: 1px solid rgba(4,55,178,0.18); border-radius: 4px; text-align: center; }}
.c5-step-num {{ font-size: 11px; font-weight: 800; color: {BLUE}; letter-spacing: 1px; margin-bottom: 4px; }}
.c5-step-name {{ font-size: 14px; font-weight: 700; }}
...
.c6-eyebrow {{ font-size: 11px; color: {CYAN}; letter-spacing: 3px; font-weight: 700; margin-bottom: 32px; }}
...
<!-- CARD 1 -->
<div class="card c1">
  <div class="c1-eyebrow">{brand} · 效率指南</div>
  <div class="c1-title">{_e(title, 40)}</div>
  <div class="c1-sub">一个<strong>AI效率工具</strong>的真实使用感受<br>从每天被周报追着跑开始</div>
  <span class="c1-tag">{brand}来了</span>
...
<div class="card c2">
  ...
  <div class="c2-text">{_e(c['hook'])}</div>
...
<div class="card c3">
  ...
  <div class="c3-headline">你中了几个？<br><span class="quote">打工人的真实痛点</span></div>
  {scenes_html}
...
<div class="card c4">
  <div class="c4-content">
    ...
    <div class="c4-text">{_e(c['insight'])}</div>
...
<div class="card c5">
  ...
  <div class="c5-bottom">
    不需要学<strong>prompt技巧</strong>，<br>
    像跟<strong>同事聊天</strong>一样自然。
  </div>
  <div class="watermark" style="color:rgba(0,0,0,0.45)">{LOGO_SVG} {brand}</div>
</div>
...
<!-- CARD 6 -->
<div class="card c6">
  <div class="c6-eyebrow">END · {brand}效率指南</div>
  <div class="c6-text-2">{_e(c['cta'][:40])}</div>
  <div class="c6-divider"></div>
  <div class="c6-sign"><strong>{brand}</strong> · AI效率工具<br>让每个打工人都用上AI</div>
  <div class="c6-cta">
    <div class="c6-cta-line">你也在用AI提效吗？<br>关注 <span class="accent">@{brand}</span> · 评论区聊聊👇</div>
  </div>
  <div class="watermark">{LOGO_SVG} {brand}</div>
</div>
...
def main():
    run_id = _make_run_id()
    default_out = str(_TEMP_BASE / "runs" / run_id)

    parser = argparse.ArgumentParser()
    parser.add_argument("--draft-json", required=True)
    parser.add_argument("--output-dir", default=default_out)
    parser.add_argument("--brand", default=os.environ.get("XHS_BRAND_NAME", ""))
    parser.add_argument("--headless", default="true", choices=["true", "false"])
    args = parser.parse_args()

    print(f"[render] run_id: {run_id}", flush=True)
    print(f"[render] output: {args.output_dir}", flush=True)

    with open(args.draft_json, "r", encoding="utf-8") as f:
        draft = json.load(f)

    html = build_html(draft, args.brand)
