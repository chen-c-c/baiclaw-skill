...
version: 1.0.0
description: |
  Create and publish Douyin (抖音) image-text posts end-to-end.
  Automatically collects today's trending topics, calls Claude/DeepSeek API to generate
  Douyin-style content (title, article, hashtags), renders carousel images,
  then publishes via Playwright to creator.douyin.com.
  Activated when users or scheduled tasks mention "生成抖音内容", "发抖音", "发布抖音",
  "抖音图集", "写抖音", "create douyin post", "publish douyin".
...
export DOUYIN_GENERATE_SCRIPT="$SKILLS_ROOT/douyin/scripts/generate.py"
export DOUYIN_CHECK_PUBLISH_SCRIPT="$SKILLS_ROOT/douyin/scripts/check_and_publish.py"
```
...
```bash
python "$DOUYIN_CHECK_PUBLISH_SCRIPT"
```

> 该脚本由 BaiClaw 定时任务触发（cron `*/5 * * * *`），无需 Agent 手动调用。
> 拉取 status=approved、platform=douyin 的文章 → 还原图片和 draft → 调用 publish.py → 回写发布结果。
